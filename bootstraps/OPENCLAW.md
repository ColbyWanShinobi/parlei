...
This is the **OpenClaw** environment configuration for Parlei. All agent logic, memory, personalities, and tools live in `../shared/` and are referenced by path. Do not duplicate any content from `../shared/` into this file.
...
- Use OpenClaw's available file and shell tools for all memory and task file operations.
...
- OpenClaw dispatches use `openclaw --print --model <model>`. If your version of OpenClaw does not yet support `--print`, set `llm_endpoint` in `shared/tools/memory_config.json` and dispatches will fall back to `llm_call.sh` automatically.
